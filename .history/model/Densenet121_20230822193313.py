...
class DenseNet_PASE(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=10, memory_efficient=False, attention_setting=[-1, -1, -1, -1], part_num=None, **kwargs):
    ...
    def forward(self, *args):
        ...
        for i in range(3):
            ...
            features = self.trans[i](features)
            self.phy_attn_a[i+1](features, ASC_part) 
        features = self.phy_attn_a[3](features, ASC_part)
        features = self.block[3](features)
        features = self.norm5(features)
        out = F.relu(features, inplace=True)
